...
marketing version and are written to `dist/`. For version `0.8.0`,
`make dmg` creates `kastan-0.8.0-macos.dmg` with a Release build stored as `Kastan.app` and displayed as `Kaštan`
...
`make source-zip` creates `kastan-0.8.0-source.zip` from the current Git `HEAD`. The archive has a top-level
`kastan-0.8.0/` directory and contains the tracked, buildable sources and documentation for every Kaštan interface.
