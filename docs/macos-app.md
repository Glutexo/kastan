...
- Search forms stay only as tall as their current controls above independently scrollable result areas, with matching
  ...
  submission with an explicit action for returning to the editable form. Main windows open at a compact 522-point
  width even when macOS has saved a wider frame, without moving the connection or station-board time mode below the
  date and time controls or clipping fields and actions at the window edge. They remain freely resizable while open.
