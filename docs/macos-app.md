...
- Selectable service-information text whose rows use semantic emoji for restrictions, routes, carriers, and
  calendar-backed operating rules. A single selection can span multiple information rows while retaining
  clickable phone-number links and the standard macOS copy command. Dated operating exceptions, including
  abbreviated ranges such as `17. to 20.VIII.` and
