...
marketing version and are written to `dist/`. For version `0.1.2`,
`make dmg` creates `kastan-0.1.2-macos.dmg` with a Release build of `Kaštan.app` for both Apple Silicon and Intel
...
`make source-zip` creates `kastan-0.1.2-source.zip` from the current Git `HEAD`. The archive has a top-level
`kastan-0.1.2/` directory and contains the tracked, buildable sources and documentation for every Kaštan interface.
