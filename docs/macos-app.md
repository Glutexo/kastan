...
- Connection searches with full-row selectable place suggestions whose icons distinguish municipalities from
  ...
  search in Connections, Departures, or Station Timetables retains its compact summary when another mode is selected
  and the passenger returns; an explicitly reopened editor remains open across the same switch. Searches also
  ...
  combines an operation popup—Only or Exclude (`pouze` or `vynechat`)—with a grouped mode popup. Trains contains only
  the four train quality classes; General contains Bus, Ship, and Other; Buses contains Local, Long-distance, and
  International bus; City transport contains Tram, Bus, Cableway, and Trolleybus. Only and Exclude are mutually
  exclusive across the complete condition: changing the operation in any row changes it in every transport row.
  Multiple Only rows form the permitted union, while multiple Exclude rows remove their modes from the complete
  catalog. A mode already used by another row is omitted.
...
- Station departures and arrivals with station-only suggestions that retain the selected station or stop identity.
  After a supported search, the compact submitted-search header and every service row offer Departures and Arrivals
  actions followed by one Station timetable action. Both board modes reuse the visible stop, day, and time;
  ...
  immediately. Every action reuses the established Option and Option-Shift destination alternatives.
- MHD station timetables with line and direction suggestions, single-day or whole-week schedules, selectable
  ...
  submitted-search header and the line-and-direction result header offer one Connections action followed by
  Departures and Arrivals. The submitted header uses its entered endpoints, while the result header uses the
  endpoints returned by the provider; departures use the direction's origin and arrivals its destination. Because a
  station timetable selects a date without a time, all three transitions use the current clock time on that service
  day. Their menu actions follow the same modifier alternatives as other
  transferred searches. An ordinary route-stop click selects that stop in the current timetable.
  Every stop, including the currently selected one, offers Connections, Departures, and Arrivals actions
  in its context menu. All use the provider-returned stop and the timetable's selected service day
  ...
  with the selected stop highlighted through the searched direction. Its context menu provides Connections,
  Departures, and Arrivals actions, with the shared modifier alternatives for choosing their destinations. The
  provider-resolved stop, date, and timetable minute also seed the arrivals query. Every complete transfer searches
  ...
  sharing action performs the same on-demand lookup and then continues with that exact dated service. Times after
  midnight and Whole week grouping follow the selected
...
- Search forms stay only as tall as their current controls above independently scrollable result areas, with matching
  ...
  to the narrowest width that places Timetable immediately beside Date and time with the compact standard gap, without
  moving the connection or station-board time mode below the date and time controls or clipping fields and actions at
  the window edge. macOS restores the last user-selected supported size thereafter; a legacy saved width below the
  minimum expands to that minimum when its window opens.
...
- The View menu's Show connection badges setting is off initially, applies to every app window, and persists across
  ...
  offers its own preview followed by calendar, PDF, and sharing actions rather
  ...
  opens the identical compact complete-route preview at the detail view's minimum supported width. Double-clicking or
  Command-clicking anywhere across a connection's time or route summary
...
- Localized IDOS calendar and PDF exports matching the app language for connection results and dated service
  ...
  requesting the calendar or PDF.
...
marketing version and are written to `dist/`. For version `0.8.2`,
`make dmg` creates `kastan-0.8.2-macos.dmg` with a Release build stored as `Kastan.app` and displayed as `Kaštan`
...
`make source-zip` creates `kastan-0.8.2-source.zip` from the current Git `HEAD`. The archive has a top-level
`kastan-0.8.2/` directory and contains the tracked, buildable sources and documentation for every Kaštan interface.
