...
- Connection searches with full-row selectable place suggestions whose icons distinguish municipalities from
  ...
  conditions. Selecting the checkbox, or entering zero as the maximum number of transfers, enables direct-only mode,
  sends zero as the maximum transfer count, and removes those three rows. Their values remain remembered; after clearing
  direct-only mode, manually adding any of the conditions restores its last value instead of its default.
...
marketing version and are written to `dist/`. For version `0.6.0`,
`make dmg` creates `kastan-0.6.0-macos.dmg` with a Release build stored as `Kastan.app` and displayed as `Kaštan`
...
`make source-zip` creates `kastan-0.6.0-source.zip` from the current Git `HEAD`. The archive has a top-level
`kastan-0.6.0/` directory and contains the tracked, buildable sources and documentation for every Kaštan interface.
