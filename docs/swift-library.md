...
`connectionCalendar` returns IDOS iCalendar text. `connectionPDF` returns the PDF bytes generated by IDOS for
one connection and accepts an explicit language for the document text.
